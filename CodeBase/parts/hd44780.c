...
_hd44780_reset_cursor(
		hd44780_t *b)
{
	//printf("Control came to _hd44780_reset_cursor");
	b->cursor = 0;
...
_hd44780_clear_screen(
		hd44780_t *b)
{
	//printf("Control came to _hd44780_clear_screen");
	memset(b->vram, ' ', 80);
	hd44780_set_flag(b, HD44780_FLAG_DIRTY, 1);
	avr_raise_irq(b->irq + IRQ_HD44780_ADDR, b->cursor);
}
...
hd44780_write_data(
		hd44780_t *b)
{
	uint32_t delay = 37; // uS(microSeconds)
		...
		hd44780_t *b )
{
	...
	//write=11;  --> O/P not correct(30H i.e 48 gets printed!)
...
static void hd44780_pin_changed(struct avr_irq_t *irq, uint32_t value, void *param)    //Original: static void hd44780_pin_changed_hook(struct avr_irq_t *irq, uint32_t value, void *param)
{
	hd44780_t *b = (hd44780_t *) param;
	...
	switch (irq->irq) {
		...
		case IRQ_HD44780_ALL:
			for (int i = 0; i < 4; i++)
				hd44780_pin_changed(b->irq + IRQ_HD44780_D4 + i,
						((value >> i) & 1), param);
			hd44780_pin_changed(b->irq + IRQ_HD44780_RS, (value >> 4), param);
			hd44780_pin_changed(b->irq + IRQ_HD44780_E,  (value >> 5), param);
			hd44780_pin_changed(b->irq + IRQ_HD44780_RW, (value >> 6), param);
...
void hd44780_init(struct avr_t *avr, struct hd44780_t * b, int width, int height)
{
	...
	printf("IRQ(Interrupt Request, HD44780) Count: %d\n",IRQ_HD44780_COUNT);

	for (int i = 0; i < IRQ_HD44780_INPUT_COUNT; i++){
	
		hd44780_pin_changed(b->irq+i,(int)hd44780_pin_changed,b);
		avr_irq_register_notify(b->irq + i, hd44780_pin_changed, b);
	}
